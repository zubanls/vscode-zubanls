...
export async function activate(context: ExtensionContext) {
	...
	context.subscriptions.push(
		workspace.onDidChangeConfiguration(async (event) => {
			if (
				event.affectsConfiguration("python.zuban") ||
				event.affectsConfiguration("zuban")
			) {
				await restartClient();
			}
		}),
...
async function findCommand(
	...
	if (pythonExecutable) {
		outputChannel.appendLine(
			`Zuban tries to work with environment ${pythonExecutable}`,
		);
		if (fs.existsSync(pythonExecutable)) {
			...
			if (fs.existsSync(zubanPath)) {
				outputChannel.appendLine(
					`Zuban found a Zuban executable at ${zubanPath}`,
				);
				return zubanPath;
			} else {
				outputChannel.appendLine(
			...
			outputChannel.appendLine(
				`Zuban executable ${pythonExecutable} does not exist`,
			);
		}
	}
