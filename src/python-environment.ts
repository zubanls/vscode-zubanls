...
export class PythonEnvironment {
    ...
    private watchExtensionChanges() {
        let hadPyEnvs = !!vscode.extensions.getExtension("ms-python.vscode-python-envs");
        let hadMsPython = !!vscode.extensions.getExtension("ms-python.python");

        this.context.subscriptions.push(
            vscode.extensions.onDidChange(() => {
                const hasPyEnvs = !!vscode.extensions.getExtension("ms-python.vscode-python-envs");
                const hasMsPython = !!vscode.extensions.getExtension("ms-python.python");
                    ...
                    .then((provider) => {
                        ...
                        if (provider) {
                            for (const listener of this.listeners) {
                                listener();
                                this.listenerDisposables.push(provider.onDidChange(listener));
                            }
                        }
                    })
                    .catch(() => {});
    ...
    async onDidChangeInterpreter(callback: () => void): Promise<vscode.Disposable> {
